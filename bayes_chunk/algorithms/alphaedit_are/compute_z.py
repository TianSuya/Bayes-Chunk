...
from bayes_chunk.segmentation import FixedSegmenter
...
def compute_z(
    ...
    segmenter = segmenter or FixedSegmenter.from_hparams(hparams)
    segments = segmenter.segment_target_ids(
        target_ids,
        model=model,
        tokenizer=tok,
    ...
    for segment in segments:
        current_target_ids = target_ids[segment.start:segment.end]
        if segment.start > 0:
            overlap = getattr(hparams, "overlap", 0)
            input_ids = torch.cat([cur_input_ids, torch.unsqueeze(current_target_ids[overlap:-1], dim=0)], dim=1)
            cur_input_ids = torch.cat([cur_input_ids, torch.unsqueeze(current_target_ids[overlap:], dim=0)], dim=1)
        else:
            input_ids = torch.cat([cur_input_ids, torch.unsqueeze(current_target_ids[:-1], dim=0)], dim=1)
            cur_input_ids = torch.cat([cur_input_ids, torch.unsqueeze(current_target_ids, dim=0)], dim=1)
        ...
        def edit_output_fn(cur_out, cur_layer):
            ...
            if cur_layer == hparams.layer_module_tmp.format(layer):
                ...
                if target_init is None:
                    if hidden.dim() == 2:
                        target_init = hidden[lookup_idxs[0]].detach().clone()
                    else:
                ...
                for idxs_pre, delta_pre in all_delta:
                    for i, idx in enumerate(idxs_pre):
                        if hidden.dim() == 2:
                            hidden[idx, :] += delta_pre
                        elif len(idxs_pre)!=len(hidden):
                            hidden[idx, i, :] += delta_pre
                        else:
                            hidden[i, idx, :] += delta_pre
                for i, idx in enumerate(lookup_idxs):
                    if hidden.dim() == 2:
                        hidden[idx, :] += delta
                    elif len(lookup_idxs)!=len(hidden):
                        hidden[idx, i, :] += delta
                    else:
                        hidden[i, idx, :] += delta
